...
def get_default_custome_number():
...
class Payment(models.Model):
    ...
    custome_number = models.CharField(
        'Номер заказа', unique=True, max_length=64,
        default=get_default_custome_number)
    status = models.CharField(
        'Результата', max_length=16, choices=STATUS.CHOICES,
    ...
    shop_amount = models.DecimalField(
        'Сумма полученная на р/с', max_digits=5, decimal_places=2,
        blank=True, null=True, help_text='За вычетом процента оператора')
    ...
    payer_code = models.CharField(
        'Номер виртуального счета', max_length=33, blank=True, null=True)
    ...
    cps_email = models.EmailField(
        'Почты плательщика', blank=True, null=True)
    ...
    performed_datetime = models.DateTimeField(
        'Выполнение запроса', blank=True, null=True)
